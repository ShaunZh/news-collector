...
## Detecting Mode: Local vs Global

First, check if running in a project directory with local viewer:

```bash
test -f "${CLAUDE_PROJECT_DIR}/index.html" && echo "MODE=local" || echo "MODE=global"
```

- **Local Mode** (`MODE=local`): Project has a local viewer setup.
  - Data saved to `data/` directory as JSON files
  - Browser-based viewer for browsing history
  - Podcast AI summary generation via conversation
  - Skip onboarding, use local storage workflow

- **Global Mode** (`MODE=global`): Standard digest delivery.
...
When `MODE=local`, use this workflow instead of the global onboarding/delivery flow.

### Trigger Commands

- `/ai` or `/follow-builders` — Fetch latest data and open browser viewer
- "帮我生成 podcast 摘要" — Generate AI summary for podcast
- "帮我生成 [date] 的 podcast 摘要" — Generate summary for specific date

### Step 1: Fetch and Save Data

Run the fetch script to get latest data:

```bash
cd ${CLAUDE_SKILL_DIR}/scripts && node fetch-local.js
```

This script:
- Fetches data from central feeds (X, podcasts, blogs)
- Filters low-value tweets
- Saves to `${CLAUDE_PROJECT_DIR}/data/YYYY-MM-DD.json`

### Step 2: Open Browser Viewer

**IMPORTANT:** Due to browser CORS restrictions, `file://` protocol cannot load local JSON files. Must use HTTP server.

Check if server is already running:
```bash
curl -s http://localhost:8888 > /dev/null && echo "SERVER=running" || echo "SERVER=stopped"
```

If server is stopped, start it:
```bash
cd ${CLAUDE_PROJECT_DIR} && python3 -m http.server 8888 &
sleep 1
```
...
### Step 3: Podcast Summary Generation (On Request)

When user requests podcast summary:

1. **Determine date**: User specified or today (`YYYY-MM-DD`)

2. **Read required files**:
   - Prompt: `${CLAUDE_PROJECT_DIR}/prompts/podcast-summary.md`
   - Data: `${CLAUDE_PROJECT_DIR}/data/{date}.json`

3. **Check content**: If no podcast or already has summary, inform user

4. **Generate summary**: Use prompt to process transcript, output JSON

5. **Save to data file**: Add `summary` field to `podcasts[]`

6. **Confirm**: Tell user to refresh browser
...
## Global Mode: Detecting Platform
